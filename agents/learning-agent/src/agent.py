...
class LearningAgent:
    ...
    async def process_event(self, event: dict) -> None:
        ...
        if "anomaly_type" in event:
            ...
            self.anomaly_types[anomaly_id] = anomaly_type
            log.info(
                "learning_agent.recorded_anomaly_type",
        ...
        # 3. RCA Event
            ...
            if incident_id in self.active_incidents:
                ...
                self.active_incidents[incident_id]["root_cause_service"] = rca_res.get(
                    "root_cause_service"
                )
                self.active_incidents[incident_id]["confidence_score"] = rca_res.get(
                    "confidence_score", 1.0
                )
                self.active_incidents[incident_id]["risk_score"] = rca_res.get("risk_score", 0.0)
                log.info(
                    "learning_agent.rca_correlated",
                    incident_id=incident_id,
                    root_cause=rca_res.get("root_cause_service"),
                )
        ...
        # 4. Decision Event
            ...
            if incident_id in self.active_incidents:
                ...
                log.info(
                    "learning_agent.decision_correlated",
                    incident_id=incident_id,
    ...
    async def evaluate_outcome(self, incident_id: str, rem_event: dict) -> None:
        ...
        remediation_action = (
            rem_event.get("action_type") or incident_info["remediation_action"] or "unknown"
        )
            ...
            if use_pg:
                cursor.execute(
                    ...
                    (
                        ...
                        incident_info["incident_title"],
                        incident_info["root_cause_service"],
                        remediation_action,
                        success,
                        recovery_time_seconds,
                        incident_info["confidence_score"],
                        effectiveness_score,
                    ),
                )
            else:
